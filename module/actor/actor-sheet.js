...
export class KnaveActorSheet extends ActorSheet {
  ...
  _onAbility_Clicked(ability) {
    ...
    switch (ability) {
      case "str":
        score = this.object.data.data.abilities.str.value;
        name = "STR";
        break;
      case "dex":
        score = this.object.data.data.abilities.dex.value;
        name = "DEX";
        break;
      case "con":
        score = this.object.data.data.abilities.con.value;
        name = "CON";
        break;
      case "int":
        score = this.object.data.data.abilities.int.value;
        name = "INT";
        break;
      case "wis":
        score = this.object.data.data.abilities.wis.value;
        name = "WIS";
        break;
      case "cha":
        score = this.object.data.data.abilities.cha.value;
        name = "CHA";
        break;
    }
  ...
  _onItemRoll(item, eventTarget) {
    if (eventTarget.title === "attack") {
      if (item.type === "weaponMelee" && !this._itemIsBroken(item)) {
        ...
        if (roll.dice[0].total === 1) this._weaponCriticalFailure(item);
  ...
  _weaponCriticalFailure(item) {
    ...
    if (item.data.data.quality.value <= item.data.data.quality.min) {
      let content =
        '<span class="knave-ability-crit knave-ability-critFailure"><b>' +
        item.name +
        "</b> broke!</span>";
      ChatMessage.create({
        user: game.user._id,
      ...
      let content =
        "<span><b>" +
        item.name +
        "</b> quality reduced to " +
        item.data.data.quality.value +
        "/" +
        item.data.data.quality.max;
      +"</span>";
      ChatMessage.create({
        user: game.user._id,
        speaker: ChatMessage.getSpeaker({ actor: this.actor }),
  ...
  _rangedAttackRoll(item) {
    if (item.data.data.ammo.value > 0) {
      const roll = this._onAbility_Clicked("dex");
      if (roll.dice[0].total === 1) this._weaponCriticalFailure(item);

      item.data.data.ammo.value -= 1;
      item.update({ "data.ammo.value": item.data.data.ammo.value });
      if (item.data.data.ammo.value <= 0) this._createNoAmmoMsg(item, true);
