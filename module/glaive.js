...
Hooks.once("init", async function () {
  ...
  Handlebars.registerHelper("talentSlots", function (talentSlots) {
    if (talentSlots.used >= talentSlots.value)
      return new Handlebars.SafeString(
        '<span class="knave-encumbered">' +
          talentSlots.used +
          "/" +
      ...
      return new Handlebars.SafeString(
        talentSlots.used + "/" + talentSlots.value
      );
  });
  ...
  Handlebars.registerHelper("isItemBroken", function (item) {
    if (item.type === "spell")
      ...
      if (item.data.data.quality) return item.data.data.quality.value <= 0;
      else return false;
    }
  });
